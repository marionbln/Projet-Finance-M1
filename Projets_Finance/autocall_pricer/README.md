...
Je constate qu’il se caractérise par le fait que le coupon n’est versé que si le produit est rappelé à une date d’observation, lorsque le sous-jacent dépasse une barrière prédéfinie. Si cette barrière n’est pas atteinte, aucun coupon n’est payé et le produit continue jusqu’à la date suivante. En l’absence de rappel jusqu’à maturité, le remboursement du capital dépend du niveau final du sous-jacent, avec généralement une protection conditionnelle du capital.

Pourquoi avoir choisi un Autocall ?

J’ai choisi un Autocall car il s’agit d’un produit très répandu sur les marchés, notamment en Europe. Je trouve que son payoff non linéaire et dépendant du chemin du sous-jacent le rend particulièrement intéressant d’un point de vue quantitatif. De plus, il n’existe pas de formule fermée simple pour le pricer, ce qui rend l’approche Monte-Carlo particulièrement adaptée et pertinente.
...
Le modèle repose sur l’absence d’opportunité d’arbitrage, des marchés frictionless, un taux sans risque constant, une volatilité constante et des rendements log-normaux du sous-jacent. Je suis conscient que certaines de ces hypothèses sont simplificatrices, en particulier celle de la volatilité constante, qui ne reflète pas toujours la réalité du marché.
...
À chaque date de constatation, je compare le niveau du sous-jacent à la barrière de rappel. Si la condition est remplie, le produit est rappelé, le remboursement du capital et des coupons est déclenché, et j’annule les flux futurs.
...
J’observe qu’une hausse de la volatilité augmente la valeur des options implicites contenues dans le produit, notamment la PDI (put down-and-in) vendue par le client. Cela permet généralement de proposer des coupons plus élevés à l’émission, mais rend le produit plus risqué en cours de vie.

Le produit est-il long ou short volatilité ?

Le client est structurellement vendeur de volatilité via la PDI. Je considère donc que le produit est globalement short volatilité.
...
J’utilise Python car il est très répandu en finance, facile à lire, rapide pour prototyper et dispose de bibliothèques adaptées au calcul scientifique et à la simulation numérique.

Comment j’assure la convergence Monte-Carlo ?

J’augmente le nombre de simulations jusqu’à ce que le prix se stabilise et j’utilise éventuellement des techniques de réduction de variance afin d’améliorer la précision sans augmenter excessivement le temps de calcul.

Comment améliorer les performances de mon code ?

Je peux vectoriser les calculs avec NumPy, utiliser des méthodes de réduction de variance, paralléliser les simulations et optimiser le nombre de pas de temps.

Esprit critique
Quelles sont les limites de mon modèle ?

Mon modèle repose sur une volatilité constante et ne prend pas en compte le smile de volatilité ni la dynamique réelle du marché. En pratique, je sais qu’on utiliserait plutôt un modèle à volatilité stochastique, comme Heston, calibré sur des données de marché.

Que ferais-je avec plus de temps ?
